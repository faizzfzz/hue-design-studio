...
class Title {
  ...
  createMesh() {
    ...
    const textHeight = (this.plane as unknown as { scale: { x: number; y: number } }).scale.y * 0.15
    const textWidth = textHeight * aspect
    this.mesh.scale.set(textWidth, textHeight, 1)
    this.mesh.position.y = -(this.plane as unknown as { scale: { x: number; y: number } }).scale.y * 0.5 - textHeight * 0.5 - 0.05
...
class Media {
  ...
  constructor(opts: {
    ...
    this.font = opts.font ?? 'bold 30px sans-serif'
    this.createShader()
    this.createMesh()
    this.createTitle()
    this.onResize()
  ...
  onResize({ screen, viewport }: { screen?: Screen; viewport?: Viewport } = {}) {
    ...
    if (viewport) {
      ...
      if ((this.plane.program.uniforms as Record<string, unknown>).uViewportSizes) {
        (this.plane.program.uniforms as Record<string, { value: number[] }>).uViewportSizes.value = [this.viewport.width, this.viewport.height]
      }
    }
    this.scale = this.screen.height / 1500
    this.plane.scale.y = (this.viewport.height * (900 * this.scale)) / this.screen.height
    this.plane.scale.x = (this.viewport.width * (700 * this.scale)) / this.screen.width
    ;(this.plane.program.uniforms as Record<string, { value: number[] }>).uPlaneSizes.value = [this.plane.scale.x, this.plane.scale.y]
    this.padding = 2
...
class App {
  ...
  constructor(
    container: HTMLElement,
    { items, bend = 1, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px sans-serif', scrollSpeed = 2, scrollEase = 0.05 }: {
  ...
  createMedias(items?: GalleryItem[], bend = 1, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px sans-serif') {
    const defaultItems: GalleryItem[] = [
      { image: 'https://picsum.photos/seed/1/800/600', text: 'Project 1' },
      { image: 'https://picsum.photos/seed/2/800/600', text: 'Project 2' },
      { image: 'https://picsum.photos/seed/3/800/600', text: 'Project 3' },
      { image: 'https://picsum.photos/seed/4/800/600', text: 'Project 4' },
      { image: 'https://picsum.photos/seed/5/800/600', text: 'Project 5' },
      { image: 'https://picsum.photos/seed/6/800/600', text: 'Project 6' },
    ]
