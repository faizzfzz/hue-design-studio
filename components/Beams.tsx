...
function createGeometry(n: number, width: number, height: number, heightSegments: number): THREE.BufferGeometry {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j <= heightSegments; j++) {
      ...
      uvs.set([uvXOff, j / heightSegments + uvYOff, uvXOff + 1, j / heightSegments + uvYOff], uOff)
      if (j < heightSegments) {
...
>(({ material, width, count, height }, ref) => {
  ...
  const geometry = useMemo(() => createGeometry(count, width, height, 100), [count, width, height])
